...
game = SyncVectorEnv(
    [make_env(args.game, i, args.game.split('/')[-1]) for i in range(args.num_envs)]
)
...
test_game = SyncVectorEnv(
    [make_test_env(args.game, args.game.split('/')[-1]) for i in range(args.num_envs)]
)
...
class Agent:
    ...
    def transform_reward(self, obses, dones):
        ...
        for j in reversed(range(len(self.reward))):
            ...
            delta = self.reward[j] + GAMMA * values[j + 1][0][0] * (1 - dones[j]) - values[j][0][0]
